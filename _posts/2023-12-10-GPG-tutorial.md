...
GPG（is short for `GNU Privacy Guard`）是一个用于加密、签名和认证数据的工具。它是由GNU计划开发的，是OpenPGP(Pretty Good Privacy)标准的一部分。

## 非对称加密（AKA. 公钥加密）

GPG采用的是非对称加密的加密方式，因此了解GPG前先了解下非对称加密。

*此外SSH、HTTPS（PKI）等也是采用非对称加密进行安全通信*。

### 生成

非对称通常通过椭圆曲线等算法产生。

非对称密钥中包含一对钥匙：私钥（也称作密钥）、公钥。私钥需要个人保管，公钥用于公开。
