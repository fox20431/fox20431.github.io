...
下类内容大部分摘录自[这个项目](https://github.com/ziishaned/learn-regex/blob/master/translations/README-cn.md)。
...
character a. But if it appears after a character set or class then it finds
the repetitions of the whole character set. For example, the regular expression
[a-z]* means: any number of lowercase letters in a row.2.3.2 The Plus
...
Regex use `()` to divide regex into multiple parts, and use `${number}` or `$number` to present them.

<pre>
"/(a) (apple)/" => This is <a href=""><strong>a apple</strong></a>
replace with `/the $2/` => This is <a href=""><strong>the apple</strong></a>
</pre>



> You can test all cases in [the regex 101 website](https://regex101.com/).
